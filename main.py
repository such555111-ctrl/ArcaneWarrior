...
log = logging.getLogger("arcane_warrior")
...
def cmd_start(message: types.Message):
    if not BASE_URL:
        bot.send_message(
            message.chat.id,
            "ARCANE WARRIOR: мини-апп ещё не сконфигурирован (не задан WEBHOOK_URL)."
        )
        return
    ...
    bot.send_message(
        message.chat.id,
        "⚡ *ARCANE WARRIOR — Россия & СНГ*\n\n"
        "Добро пожаловать в ARCANE WARRIOR! Премиальное экспедиционное снаряжение "
        "для кемпинга и автопутешествий: багажники, автобоксы, маркизы и палатки.\n\n"
...
def customer_cancel_order(oid):
    ...
    for admin_id in ADMIN_CHAT_IDS:
        try:
            bot.send_message(admin_id, f"❌ Клиент отменил заказ AW-{oid}")
        except Exception as e:
            log.warning("Не удалось уведомить администратора %s: %s", admin_id, e)
...
def admin_message_customer(oid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Сообщение от ARCANE WARRIOR по заказу AW-{oid}:\n\n{text}"
        )
    except Exception as e:
...
def admin_message_question(qid):
    ...
    try:
        bot.send_message(
            chat_id,
            f"💬 Ответ от ARCANE WARRIOR по товару «{q.get('product_name','')}»:\n\n{text}"
        )
    except Exception as e:
